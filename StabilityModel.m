...
%     sLength = StabilityModel.soleusLength(x(1));
%     taLength = StabilityModel.tibialisLength(x(1));
        
    sNormT = S.getNormalizedLengthSE(StabilityModel.soleusLength(x(1)), x(3));
    taNormT = TA.getNormalizedLengthSE(StabilityModel.tibialisLength(x(1)), x(4));
    
    Ms = (S.f0M * S.forceLengthSE(sNormT)* dS);
    Mta = (TA.f0M * TA.forceLengthSE(taNormT) * dTA);

    x1dot = x(2);
    x2dot = (Ms - Mta + getGravityMoment(x(1)))/Iankle;
    x3dot = HillTypeMuscle.getVelocity(aS, x(3), sNormT);
    x4dot = HillTypeMuscle.getVelocity(aTA, x(4), taNormT);
    
    dx_dt = [x1dot; x2dot; x3dot; x4dot];
